...
var handler = async (event, context) => {
  ...
  try {
    ...
    const arn_hash = createHash("sha256").update(original_function_arn).digest("hex");
    const arn_hash_prefix = arn_hash.substring(0, 16);
    const channel_name_for_path = `${appsync_channel_namespace}-requests-${arn_hash_prefix}`;
    const channel_identifier_for_logging_and_response = `/${channel_name_for_path}`;
    const appsync_message_payload = {
      events: [{ data: JSON.stringify(event) }]
      // Wrap stringified event in an object with a 'data' field
    };
