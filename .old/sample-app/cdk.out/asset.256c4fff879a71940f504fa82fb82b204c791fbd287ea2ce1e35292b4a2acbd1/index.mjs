...
var handler = async (event, context) => {
  ...
  try {
    ...
    const appsync_base_path = parsed_url.pathname;
    const arn_hash = createHash("sha256").update(original_function_arn).digest("hex");
    const arn_hash_prefix = arn_hash.substring(0, 16);
    const channel_name_for_body = `${appsync_channel_namespace}-requests-${arn_hash_prefix}`;
    const channel_identifier_for_logging_and_response = `/${channel_name_for_body}`;
    const appsync_request_body = {
      channel: channel_name_for_body,
      events: [JSON.stringify(event)]
      // The original event, stringified
    };
    ...
    const request_to_sign = {
      method: "POST",
      hostname,
      path: appsync_base_path,
      // Use the base path from APPSYNC_EVENT_API_URL, e.g. /event
      ...
      body: JSON.stringify(appsync_request_body)
    };
    const signed_request = await signer.sign(request_to_sign);
    console.log(
      `LAMBDA PROXY: Publishing to AppSync. Channel in body: ${channel_name_for_body}, Path: ${appsync_base_path}`
    );
    const fetch_response = await fetch(`${appsync_event_api_url}`, {
      // URL should be base AppSync Event API URL
      method: signed_request.method,
      headers: signed_request.headers,
      body: signed_request.body
    });
    const response_body = await fetch_response.text();
    if (!fetch_response.ok) {
      console.error(
        `LAMBDA PROXY: Failed to publish event to AppSync. Status: ${fetch_response.status}, Response: ${response_body}`
      );
      return {
        statusCode: 200,
        ...
        body: JSON.stringify({
          ...
          appsync_status: fetch_response.status,
      ...
      console.log(
        `LAMBDA PROXY: Successfully published event to AppSync. Status: ${fetch_response.status}, Response: ${response_body}`
      );
    }
    return {
      statusCode: 200,
