...
var handler = async (event, context) => {
  ...
  try {
    ...
    const sanitized_arn = original_function_arn.replace(/[^a-zA-Z0-9_\-]/g, "_");
    const channel = `/${appsync_channel_namespace}/${sanitized_arn}/requests`;
    const appsync_message_payload = {
      ...
      channel,
      events: [event]
      // Pass the original APIGatewayProxyEvent directly
    };
