...
async function sign_and_publish_event(
  ...
  try {
    console.log('[DEBUG] Request to sign (pre-signature):', {
      ...
      bodyPreview:
        stringified_request_body.substring(0, 200) +
        (stringified_request_body.length > 200 ? '...' : '')
    })
...
> = async (event, context) => {
  ...
  if (!appsync_event_api_url_env) {
    // APPSYNC_CHANNEL_NAMESPACE no longer strictly needed here for fixed channel
    console.error(
  ...
  if (publish_result.success) {
    ...
    return {
      ...
      body: JSON.stringify({
        message:
          'Stub Lambda successfully processed and published the event for local dev server.',
        invocationId: invocation_id,
    ...
    return {
      ...
      body: JSON.stringify({
        message:
          'Stub Lambda FAILED to publish the event for local dev server.',
        invocationId: invocation_id,
