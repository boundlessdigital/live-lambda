...
-   **Deploy AWS infrastructure** (AppSync API, Lambda Layer, etc.):
    ...
    ```
    *You can target specific stacks if needed, e.g., `pnpm run dev deploy AppSyncStack LiveLambdaLayerStack --profile <your-profile-name>`.*

-   **Start the local development server** (listens for proxied Lambda invocations):
    ```bash
    pnpm run dev server --profile <your-profile-name>
    ```
    *Ensure the server is running before your Lambda function (with the Live Lambda layer) is invoked.*
